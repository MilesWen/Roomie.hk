...
class Room(models.Model):
    ...
    def __str__(self): # __str__ for Python 3, __unicode__ for Python 2
        return self.room_type + " id:" + str(self.id) + ", owner:" + self.user.__str__()
...
class Bookmark(models.Model):
    ...
    class Meta:
        unique_together = (("uid", "roomid"),)
    
    def __str__(self):
        return str("user:" + self.user.__str__() + " bookmark room:" + str(self.room.id))
    
class Friend(models.Model):
    uid1 = models.ForeignKey(User, related_name='user1')
    uid2 = models.ForeignKey(User)